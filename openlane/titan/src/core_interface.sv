...
module core_interface # (
    ...
    // these values should be changed when insanciating a new module
    // these are only here for cocotb/icarus verilog to compile
    parameter TOTAL_INPUTS = 0,
    parameter TOTAL_OUTPUTS = 0,
    parameter START_ADDRESS = 'hFFFFFF,
    parameter END_ADDRESS = 'hFFFFFF
) (
    input wire clk_i,
    ...
    logic [VALUE_WIDTH-1:0] input_memory [0:3];  // use params to calculate required depth
    ...
    simple_neuron uut_simple_neuron (
        .clock(clk_i), .x0(input_memory[0]), .x1(input_memory[1]), .x2(input_memory[2]), .x3(input_memory[3]),
        .r(output_memory)
    );
